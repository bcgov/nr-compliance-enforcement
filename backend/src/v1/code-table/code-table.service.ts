...
import { PreventionEducationType } from "src/types/models/code-tables/prevention-education-type";
  ...
  getCodeTableByName = async (table: string, token?: string): Promise<BaseCodeTable[]> => {
    ...
    switch (table) {
      ...
      case "assessment-type": {
        ...
        console.log("testing4: " + JSON.stringify(data));
        const assessmentTypeCodes = data.HWCRAssessmentActions.map(
          ({
            ...
            console.log("testing3: " + JSON.stringify(table));
            return table;
          }
        );
      ...
      case "prevention-education-type": {
        ...
        const preventionEducationTypeCodes = data.HWCRPreventionActions.map(
          ({
            actionCode,
            shortDescription,
            longDescription,
            ...
            const table: PreventionEducationType = {
              preventionEducationType: actionCode,
              shortDescription: shortDescription,
              longDescription: longDescription,
              displayOrder: displayOrder,
              isActive: activeIndicator
        ...
        return preventionEducationTypeCodes;
      }
      case "sex": {
        const { data } = await get(token, { 
