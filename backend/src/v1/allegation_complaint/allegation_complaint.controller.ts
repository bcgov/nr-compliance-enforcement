...
export class AllegationComplaintController {
  ...
  search(@Query('sortColumn') sortColumn: string, @Query('sortOrder') sortOrder: string, @Query('community') community: string, @Query('zone') zone: string,
   ...
   @Query('inProgressInd') inProgressInd: string, @Query('incidentReportedStart') incidentReportedStart: string, @Query('incidentReportedEnd') incidentReportedEnd: string, @Query('status') status) {
    return this.allegationComplaintService.search(sortColumn, sortOrder, community, zone, region, officerAssigned, violationCode, inProgressInd, incidentReportedStart, incidentReportedEnd, status);
  }
