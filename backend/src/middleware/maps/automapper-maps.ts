...
//-- models (dto for now)
import { Agency, Attractant, NatureOfComplaint, OrganizationCodeTable, Species, Violation } from "src/types/models/code-tables";
...
export const applyOrganizationMap = (mapper: Mapper) => { 
   createMap<CosGeoOrgUnit, OrganizationCodeTable>(
      mapper,
      "CosGeoOrgUnit", //-- source
      "OrganizationCodeTable", //-- destination
...
export const applySpeciesMap = (mapper: Mapper) => { 
   createMap<SpeciesCode, Species>(mapper, "SpeciesCode", "SpeciesDto", 
      forMember(destination => destination.species, mapFrom(source => source.species_code)),
      forMember(destination => destination.legacy, mapFrom(source => source.legacy_code)),
      forMember(destination => destination.shortDescription, mapFrom(source => source.short_description)),
      forMember(destination => destination.longDescription, mapFrom(source => source.long_description)),
      forMember(destination => destination.isActive, mapFrom(source => source.active_ind)),
      forMember(destination => destination.displayOrder, mapFrom(source => source.display_order)),   
   )
}

export const applyNatureOfComplaintMap = (mapper: Mapper) => {
   createMap<HwcrComplaintNatureCode, NatureOfComplaint>(mapper, "NatureOfComplaintCode", "NatureOfComplaintDto", 
      forMember(destination => destination.natureOfComplaint, mapFrom(source => source.hwcr_complaint_nature_code)),
      forMember(destination => destination.shortDescription, mapFrom(source => source.short_description)),
      forMember(destination => destination.longDescription, mapFrom(source => source.long_description)),
      forMember(destination => destination.displayOrder, mapFrom(source => source.display_order)),
      forMember(destination => destination.isActive, mapFrom(source => source.active_ind)),
...
export const applyAttractantCodeMap = (mapper: Mapper) => { 
   createMap<AttractantCode, Attractant>(mapper, "AttractantCode", "AttractantDto", 
      forMember(destination => destination.attractant, mapFrom(source => source.attractant_code)),
      forMember(destination => destination.shortDescription, mapFrom(source => source.short_description)),
      forMember(destination => destination.longDescription, mapFrom(source => source.long_description)),
      forMember(destination => destination.displayOrder, mapFrom(source => source.display_order)),
      forMember(destination => destination.isActive, mapFrom(source => source.active_ind)),
...
export const applyAgencyCodeMap = (mapper: Mapper) => { 
   createMap<AgencyCode, Agency>(mapper, "AgencyCode", "AgencyCodeDto", 
      forMember(destination => destination.agency, mapFrom(source => source.agency_code)),
      forMember(destination => destination.shortDescription, mapFrom(source => source.short_description)),
      forMember(destination => destination.longDescription, mapFrom(source => source.long_description)),
      forMember(destination => destination.displayOrder, mapFrom(source => source.display_order)),
      forMember(destination => destination.isActive, mapFrom(source => source.active_ind)),
...
export const applyAttractantReferenceMap = (mapper: Mapper) => {
...
export const applyViolationCodeMap = (mapper: Mapper) => { 
   createMap<ViolationCode, Violation>(mapper, "ViolationCode", "ViolationCodeDto", 
      forMember(destination => destination.violation, mapFrom(source => source.violation_code)),
      forMember(destination => destination.shortDescription, mapFrom(source => source.short_description)),
      forMember(destination => destination.longDescription, mapFrom(source => source.long_description)),
      forMember(destination => destination.displayOrder, mapFrom(source => source.display_order)),
      forMember(destination => destination.isActive, mapFrom(source => source.active_ind)),
...
export const applyWildlifeComplaintMap = (mapper: Mapper) => { 
   applySpeciesMap(mapper)
   applyNatureOfComplaintMap(mapper)
   applyAttractantReferenceMap(mapper)
   applyAttractantCodeMap(mapper)
   applyAgencyCodeMap(mapper)
   applyOrganizationMap(mapper)

   createMap<HwcrComplaint, WildlifeComplaintDto>(mapper, "WildlifeComplaint", "WildlifeComplaintDto", 
      forMember(destination => destination.id, mapFrom(source => source.complaint_identifier.complaint_identifier)),
      forMember(destination => destination.details, mapFrom(source => {
         const { complaint_identifier: complaint } = source;
         return complaint.detail_text !== null ? complaint.detail_text : ""
      })),
      forMember(destination => destination.name, mapFrom(source => 
         {
            const { complaint_identifier: complaint } = source;
            return complaint.caller_name !== null ? complaint.caller_name : ""
         })),
      forMember(destination => destination.address, mapFrom(source => {
         const { complaint_identifier: complaint } = source;
         return complaint.caller_address !== null ? complaint.caller_address : ""
      })),
      forMember(destination => destination.email, mapFrom(source => {
         const { complaint_identifier: complaint } = source;
         return complaint.caller_email !== null ? complaint.caller_email : ""
      })),
      forMember(destination => destination.phone1, mapFrom(source => {
         const { complaint_identifier: complaint } = source;
         return complaint.caller_phone_1 !== null ? complaint.caller_phone_1 : ""
      })),
      forMember(destination => destination.phone2, mapFrom(source => { 
         const { complaint_identifier: complaint } = source;
         return complaint.caller_phone_2 !== null ? complaint.caller_phone_2 : ""
      })),
      forMember(destination => destination.phone3, mapFrom(source => { 
         const { complaint_identifier: complaint } = source;
         return complaint.caller_phone_3 !== null ? complaint.caller_phone_3 : ""
      })),
      forMember(destination => destination.location, mapFrom(source => { 
         const { complaint_identifier: { location_geometry_point: { type: locationType, coordinates }} } = source
         return { type: locationType, coordinates };
      })),
      forMember(destination => destination.locationSummary, mapFrom(source => source.complaint_identifier.location_summary_text)),
      forMember(destination => destination.locationDetail, mapFrom(source => source.complaint_identifier.location_detailed_text)),
      forMember(destination => destination.status, mapFrom(source => { 
         const  { complaint_identifier: { complaint_status_code: { complaint_status_code} } } = source
      return complaint_status_code
      })),
      forMember(destination => destination.referredBy, mapFrom(source => { 
         const { complaint_identifier: { referred_by_agency_code: agency }} = source
         if(agency !== null){ 
      ...
      forMember(destination => destination.ownedBy, mapFrom(source => {
         const { complaint_identifier: { owned_by_agency_code: agency }} = source
         if(agency !== null){ 
            const code = mapper.map<AgencyCode, Agency>(agency, "AgencyCode", "AgencyCodeDto")
            return code.agency;
         } 
      ...
      forMember(destination => destination.referredByAgencyOther, mapFrom(source => source.complaint_identifier.referred_by_agency_other_text)),
      forMember(destination => destination.incidentDateTime, mapFrom(source => source.complaint_identifier.incident_utc_datetime)),
      forMember(destination => destination.reportedOn, mapFrom(source => source.complaint_identifier.incident_reported_utc_timestmp)),
      forMember(destination => destination.organization, mapFrom(source => { 
         const { complaint_identifier: { cos_geo_org_unit: sourceOrganization }} = source
         return mapper.map<CosGeoOrgUnit, OrganizationCodeTable>(sourceOrganization, "CosGeoOrgUnit", "OrganizationCodeTable")
      })),
      forMember(destination => destination.delegates, mapFrom(source => { 
         const { complaint_identifier: { person_complaint_xref: people }} = source
      ...
      forMember(destination => destination.hwcrId, mapFrom(src => src.hwcr_complaint_guid)),
      forMember(destination => destination.attractants, mapFrom(src => { 
         if(src.attractant_hwcr_xref !== null){
            return mapper.mapArray<AttractantHwcrXref, AttractantXrefDto>(src.attractant_hwcr_xref, "AttractantXref", "AttractantXrefDto")
         }

         return []
      })),
      forMember(destination => destination.otherAttractants, mapFrom(src => { 
      ...
      forMember(destination => destination.species, mapFrom(src => { 
      ...
      forMember(destination => destination.natureOfComplaint, mapFrom(src => { 
         const item = mapper.map<HwcrComplaintNatureCode, NatureOfComplaint>(src.hwcr_complaint_nature_code, "NatureOfComplaintCode", "NatureOfComplaintDto")
         if(item !== null){
...
export const applyAllegationComplaintMap = (mapper: Mapper) => { 
   applyViolationCodeMap(mapper)
   applyAgencyCodeMap(mapper)
   applyOrganizationMap(mapper)

   createMap<AllegationComplaint, AllegationComplaintDto>(mapper, "AllegationComplaint", "AllegationComplaintDto", 
      forMember(destination => destination.id, mapFrom(source => source.complaint_identifier.complaint_identifier)),
      forMember(destination => destination.details, mapFrom(source => {
         const { complaint_identifier: complaint } = source;
         return complaint.detail_text !== null ? complaint.detail_text : ""
      ...
      forMember(destination => destination.ersId, mapFrom(src => src.allegation_complaint_guid)),
      forMember(destination => destination.violation, mapFrom(src => { 
      ...
      forMember(destination => destination.isInProgress, mapFrom(src => src.in_progress_ind)),
      forMember(destination => destination.wasObserved, mapFrom(src => src.observed_ind)),
      forMember(destination => destination.violationDetails, mapFrom(src => src.suspect_witnesss_dtl_text)),
   )
}