...
BEGIN
    ...
    _upd_location_detailed_text := update_complaint_data ->> 'update_location_decription';
    ...
    _update_caller_phone_2 := update_complaint_data ->> 'update_alternate_phone';
    _update_caller_phone_3 := update_complaint_data ->> 'update_alternate_2_phone';
  ...
  if ((_update_reported_by_code <> _original_reported_by_code) and (_update_reported_by_code <> current_complaint_record.reported_by_code)) then
	update complaint
	set reported_by_code = _update_reported_by_code, update_user_id = _update_userid, update_utc_timestamp = _update_utc_timestamp
	where complaint_identifier = complaint_identifier;
   end if;
  ...
  if ((_update_reported_by_other_text <> _original_reported_by_other_text) and (_update_reported_by_other_text <> current_complaint_record.reported_by_other_text)) then
	update complaint
	set reported_by_other_text = _update_reported_by_other_text, update_user_id = _update_userid, update_utc_timestamp = _update_utc_timestamp
	where complaint_identifier = complaint_identifier;
   end if;
 ...
 if (_parent_report_type = 'ERS') then
       SELECT *
      FROM   PUBLIC.insert_and_return_code( update_complaint_data->>'violation_type', 'violatncd' )
