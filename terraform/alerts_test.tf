...
resource "sysdig_monitor_alert_v2_prometheus" "nr_database_test_uptime_score" {
  ...
  severity = "high"
  query = "sysdig_container_up{kube_cluster_name=\"silver\",kube_namespace_name=\"c1c7ed-test\",kube_statefulset_label_postgres_operator_crunchydata_com_cluster=\"postgres-crunchy-test\"} < 0.5"
  enabled = true
  duration_seconds = 180
  notification_channels {
    id = sysdig_monitor_notification_channel_email.test_environment_alerts.id
...
resource "sysdig_monitor_alert_v2_prometheus" "nr_database_test_storage_usage" {
  ...
  severity = "high"
  query = "sysdig_fs_used_percent{kube_cluster_name=\"silver\",kube_namespace_name=\"c1c7ed-test\",kube_statefulset_label_postgres_operator_crunchydata_com_cluster=\"postgres-crunchy-test\"} > 65"
  enabled = true
