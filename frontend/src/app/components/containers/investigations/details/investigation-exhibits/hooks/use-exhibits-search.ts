...
const serializeSearchValueToUrl = (key: keyof ExhibitsSearchParams, value: any): string | undefined => {
  if (value == null) {
    return undefined;
  }
...
export const useExhibitsSearch = () => {
  const [searchParams, setSearchParams] = useSearchParams();

  const searchValues: ExhibitsSearchParams = useMemo(
    () => ({
  ...
  const setValues = useCallback(
    (values: Partial<ExhibitsSearchParams>) => {
      setSearchParams(
        (currentParams) => {
          const newParams = new URLSearchParams(currentParams);

          Object.entries(values).forEach(([key, value]) => {
            const serialized = serializeSearchValueToUrl(key as keyof ExhibitsSearchParams, value);
  ...
  const clearValues = useCallback(
    (keys: keyof ExhibitsSearchParams | (keyof ExhibitsSearchParams)[]) => {
      const keysArray = Array.isArray(keys) ? keys : [keys];
      const valuesToClear: Partial<ExhibitsSearchParams> = {};
