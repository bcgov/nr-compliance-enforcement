...
type Props = {
  ...
  handleToggleView: (view: "map" | "list") => void;
  activeView: "map" | "list";
};

export const HwcrComplaintFilterContainer: FC<Props> = ({
  getCollapseProps,
  isExpanded,
  regionCodeFilter,
  setRegionCodeFilter,
  ...
  const [hwcrNatureOfComplaintCodes, setHwcrNatureOfComplaintCodes] = useState<
    Option[]
  >([] as Array<Option>);
  const [speciesCodes, setSpeciesCodes] = useState<Option[]>(
    [] as Array<Option>
  );
  const [complaintStatusCodes, setComplaintStatusCodes] = useState<Option[]>(
    [] as Array<Option>
  );
    ...
    async function fetchCodes() {
      ...
      if (token) {
        ...
        await axios
          ...
          .then((response) => {
            const transformedOptions: Option[] = response.data.map(
              (item: any) => ({
        ...
        await axios
          .get(`${config.API_BASE_URL}/v1/officer`)
          .then((response) => {
            const transformedOptions: Option[] = response.data.map(
              (item: any) => ({
                value: item.person_guid.person_guid, // Assuming each item has an 'id' property
                label:
                  item.person_guid.first_name.substring(0, 1) +
                  ". " +
                  item.person_guid.last_name, // Assuming each item has a 'name' property
              })
            );
        ...
        await axios
          .get(`${config.API_BASE_URL}/v1/hwcr-complaint-nature-code`)
          .then((response) => {
            const transformedOptions: Option[] = response.data.map(
              (item: any) => ({
        ...
        await axios
          .get(`${config.API_BASE_URL}/v1/species-code`)
          .then((response) => {
            const transformedOptions: Option[] = response.data.map(
              (item: any) => ({
        ...
        await axios
          .get(`${config.API_BASE_URL}/v1/complaint-status-code`)
          .then((response) => {
            const transformedOptions: Option[] = response.data.map(
              (item: any) => ({
  ...
  const complaintStatusClass =
    complaintStatusFilter === null || complaintStatusFilter.value === ""
      ? "hidden"
      : "comp-filter-pill";
  const dateStatusClass =
    startDateFilter === null || startDateFilter === undefined
      ? "hidden"
      : "comp-filter-pill";
  const speciesClass =
    speciesCodeFilter === null || speciesCodeFilter.value === ""
      ? "hidden"
      : "comp-filter-pill";
  const natureOfComplaintClass =
    natureOfComplaintFilter === null || natureOfComplaintFilter.value === ""
      ? "hidden"
      : "comp-filter-pill";
  const officerClass =
    officerFilter === null || officerFilter.value === ""
      ? "hidden"
      : "comp-filter-pill";
  const areaClass =
    areaCodeFilter === null || areaCodeFilter.value === ""
      ? "hidden"
      : "comp-filter-pill";
  const zoneClass =
    zoneCodeFilter === null || zoneCodeFilter.value === ""
      ? "hidden"
      : "comp-filter-pill";
  const regionClass =
    regionCodeFilter === null || regionCodeFilter.value === ""
      ? "hidden"
      : "comp-filter-pill";
  let datePillText = "";
      ...
      <div className="comp-filter-pill-container">
        ...
        <div className="comp-filter-pills">
          <div className={complaintStatusClass}>
            <button
              type="button"
              className="btn btn-primary comp-filter-btn"
              id="comp-status-filter"
            >
              {complaintStatusFilter?.label}
              <button
                type="button"
                className="btn-close btn-close-white filter-pill-close"
                aria-label="Close"
                onClick={() => setComplaintStatusFilter(null)}
              ></button>
            </button>
          </div>
          <div className={dateStatusClass}>
            <button type="button" className="btn btn-primary comp-filter-btn">
              {datePillText}
              <button
                type="button"
                className="btn-close btn-close-white filter-pill-close"
          ...
          <div className={speciesClass}>
            <button type="button" className="btn btn-primary comp-filter-btn">
              {speciesCodeFilter?.label}
              <button
                type="button"
                className="btn-close btn-close-white filter-pill-close"
                aria-label="Close"
                onClick={() => {
                  setSpeciesCodeFilter(null);
                }}
              ></button>
            </button>
          </div>
          <div className={natureOfComplaintClass}>
            <button type="button" className="btn btn-primary comp-filter-btn">
              {natureOfComplaintFilter?.label}
              <button
                type="button"
                className="btn-close btn-close-white filter-pill-close"
                aria-label="Close"
                onClick={() => setNatureOfComplaintFilter(null)}
              ></button>
            </button>
          </div>
          <div className={officerClass}>
            <button type="button" className="btn btn-primary comp-filter-btn">
              ...
              <button
                ...
                className="btn-close btn-close-white filter-pill-close"
                aria-label="Close"
                onClick={() => setOfficerFilter(null)}
              ></button>
            </button>
          </div>
          <div className={areaClass}>
            <button type="button" className="btn btn-primary comp-filter-btn">
              {areaCodeFilter?.label}
              <button
                type="button"
                className="btn-close btn-close-white filter-pill-close"
                aria-label="Close"
                onClick={() => setAreaCodeFilter(null)}
              ></button>
            </button>
          </div>
          <div className={zoneClass}>
            <button
              ...
              id="comp-zone-filter"
            >
              {zoneCodeFilter?.label}
              <button
                type="button"
                className="btn-close btn-close-white filter-pill-close"
                aria-label="Close"
                id="comp-zone-close"
                onClick={() => setZoneCodeFilter(null)}
              ></button>
            </button>
          </div>
          <div className={regionClass}>
            <button type="button" className="btn btn-primary comp-filter-btn">
              {regionCodeFilter?.label}
              <button
                type="button"
                className="btn-close btn-close-white filter-pill-close"
                aria-label="Close"
