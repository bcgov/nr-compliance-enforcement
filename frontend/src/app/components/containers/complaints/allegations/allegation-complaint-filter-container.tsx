...
export const AllegationComplaintFilterContainer: FC<Props> = ({
  getCollapseProps,
  isExpanded,
  regionCodeFilter,
  setRegionCodeFilter,
  ...
  const [violationCodes, setViolationCodes] = useState<Option[]>(
    [] as Array<Option>
  );
  ...
  useEffect(() => {
    async function fetchCodes() {
      ...
      if (token) {
        ...
        await axios
          .get(`${config.API_BASE_URL}/v1/violation-code`)
          .then((response) => {
            const transformedOptions: Option[] = response.data.map(
              (item: any) => ({
  ...
  const violationClass =
    violationFilter === null || violationFilter.value === ""
      ? "hidden"
      : "comp-filter-pill";
  const officerClass =
