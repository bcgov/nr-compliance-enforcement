...
def load_raw_index(cfg: Config, group: str) -> list[dict]:
    """Read the raw-snippet index sidecar (``working_dir/<group>/index.yaml``); ``[]`` if absent."""
...
def render_from_disk(cfg: Config, group: str, metas: list[dict]) -> str:
    """Reconstruct a document from the raw files in ``working_dir/<group>/``, ordered by ``line``.

    ``metas`` are the raw-index entries for one source (file/line/codeblock_tag), from the
    sidecar index written alongside the raws.
    """
    parts: list[str] = []
