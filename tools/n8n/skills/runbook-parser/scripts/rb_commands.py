...
* :func:`cmd_check`  — drift detection: round-trip + seal recheck (read-only;
  exit 2 on drift).
* :func:`cmd_seal`   — record the per-source content seal + snapshot the cleaned
  snippets.
* :func:`cmd_diff`   — show how cleaned snippets diverged from their seal
  (read-only back-port aid).

Status write-back replaces only the machine-owned ``status:`` block (from its
marker to end-of-file), leaving the commented ``spec`` untouched, and preserves
agent-set ``cleaned`` annotations across ingests.
"""
...
def select_sources(cfg: Config, groups: list[str], sources: list[str]) -> list[Source]:
  """Return the sources to act on, narrowed by ``--group`` and ``--source``.

  ``--group`` matches the exact group and ``--source`` the exact unique source
  name. Both filters apply together (AND).
  """
  out = []
...
def group_order(cfg: Config) -> list[str]:
  """Group names in first-seen config order.

  Sources sharing a group merge into one.
  """
  seen: list[str] = []
  for s in cfg.sources:
...
def source_metas(cfg: Config, src: Source) -> list[dict]:
  """The raw-index entries belonging to one source.

  Read from ``working_dir/<group>/index.yaml``.
  """
...
def cmd_ingest(cfg: Config, args) -> int:
  """Resolve sources, extract raws, and rewrite the committed ``status``.

  Raws are extracted into ``working_dir``. Raws (and their index sidecar) go under
  ``working_dir/<group>/`` — ephemeral unless ``working_dir`` is a committed path.
  With ``--group``/``--source`` this is a *partial* ingest: only the selected
  sources are fetched and written; pruning, the index, and the ``sources`` status
  are scoped to them (other sources are left intact).
  """
  ...
  for src in selected:
    ...
    if body is None:
      ...
      source_status[src.group].append(
        {"name": src.name, "ref": src.name, "kind": "none", "drift": "n/a"}
      )
      continue
  ...
  for gname in group_order(cfg):
    ...
    write_group(
      ...
      groups[gname],
      args.dry_run,
      processed_docnames.get(gname) if partial else None,
    )
...
def write_group(
  ...
  dry_run: bool,
  prune_docnames: set | None = None,
) -> None:
  """Write a group's raw files + index sidecar and prune stale ones.

  Files are written under ``working_dir/<group>/``. ``prune_docnames`` (partial
  ingests) scopes pruning to files whose source was processed this run — files
  from the ``<docname>_`` prefix of any other source are left alone.
  """
  ...
  if group_dir.is_dir():
    for existing in sorted(group_dir.glob("*")):
      if (
        ...
        or existing.name == "index.yaml"
        or "_" not in existing.stem
      ):
        continue  # skip the index, directories, and non-raw files
      if (
        prune_docnames is not None
        and existing.stem.split("_", 1)[0] not in prune_docnames
      ):
...
def _write_raw_index(
  ...
  group: str,
  snippets: list[Snippet],
  prune_docnames: set | None,
  dry_run: bool,
) -> None:
  """Write the raw-index sidecar that reconstruction reads.

  The sidecar lives at ``working_dir/<group>/index.yaml``. Partial ingests merge:
  index entries for sources not processed this run are kept.
  """
  fresh = [
    {
      "file": s.file,
      "section": s.section,
      "codeblock_tag": s.codeblock_tag,
      "from": s.source_ref,
      "line": s.line,
    }
  ...
  if prune_docnames is not None:
    kept = [
      m
      for m in load_raw_index(cfg, group)
      if m.get("file", "").split("_", 1)[0] not in prune_docnames
    ]
...
def build_status(cfg: Config, source_status, processed_refs=None) -> dict:
  """Assemble the committed ``status`` block from parser and agent-maintained state.

  Per group: parser-written ``sources`` plus the agent-maintained
  ``cleaned``/``skipped`` lists, preserved across ingests.

  With ``processed_refs`` (partial ingest), ``sources`` entries for sources not
  processed this run are kept; ``cleaned``/``skipped`` are always preserved whole.
  """
  ...
  for gname in group_order(cfg):
    ...
    if processed_refs is not None:
      kept = [
        s for s in (eg.get("sources") or []) if s.get("ref") not in processed_refs
      ]
...
def write_status(cfg: Config, status: dict, dry_run: bool) -> None:
  """Replace the config's ``status:`` block, leaving ``spec`` byte-for-byte.

  The block spans from its marker to EOF. Only the machine-owned ``status:``
  block is re-emitted; the commented ``spec`` above the marker is preserved
  exactly, since PyYAML's load/dump would otherwise drop its comments.
  """
  ...
  if dry_run:
    info(
      "  [dry-run] would rewrite status block "
      f"({len(status.get('groups', {}))} group(s))"
    )
    return
...
def cmd_seal(cfg: Config, args) -> int:
  """Record each selected source's content seal in ``status``.

  The seal is a sha of doc + cleaned code. Run after cleaning a source. The seal
  is preserved across ingests; ``check`` recomputes it and flags a mismatch as
  snippet-code drift -- a dev changed a cleaned command (or the doc moved), and
  the change may need backporting to the source doc.
  """
  ...
  for src in select_sources(cfg, args.group or [], args.source or []):
    ...
    if digest is None:
      warn(
        f"  {src.name}: nothing to seal (no cleaned snippets, or doc/snippet missing)"
      )
      continue
...
def cmd_include(cfg: Config, args) -> int:
  ...
  The shortcut for a snippet you authored by hand (not produced from a raw): it
  records the snippet under the source's ``cleaned`` list in ``status`` and seals.
  For a snippet with no source doc, target a placeholder source (one with no
  ``url``/``file``) -- the seal then covers the snippet alone.
  """
  sources = select_sources(cfg, args.group or [], args.source or [])
  if len(sources) != 1:
    raise RunbookError(
      "include needs exactly one --source (the source to associate with)"
    )
  src = sources[0]
  ...
  if missing:
    ...
    raise RunbookError(
      f"snippet(s) not found under {where}: {', '.join(missing)} (write them first)"
    )
...
def cmd_check(cfg: Config, args) -> int:
  """Compare each source's reconstruction to its doc; exit 2 on drift.

  Prints a diff for any drifted source.
  """
  drift = 0
  for src in select_sources(cfg, args.group or [], args.source or []):
    if src.kind() == "none":
...
def _check_seals(cfg: Config, args) -> int:
  """Recompute each selected source's seal; report snippet-code drift.

  Reported drift is a backport candidate.
  """
  ...
  for src in select_sources(cfg, args.group or [], args.source or []):
    ...
    if current is None:
      ...
      print(
        f"  {src.name}: SNIPPET CODE DRIFT - doc/cleaned changed since seal; "
        "review backport"
      )
      worst = max(worst, 2)
    else:
...
def cmd_diff(cfg: Config, args) -> int:
  """Show how cleaned snippets diverged from their seal, to drive a doc back-port.

  Read-only. For each selected source whose seal no longer matches, prints each
  cleaned snippet's change as a unified diff against the seal-time snapshot (the
  dev's edit) and points at the source doc, so the agent can propose the doc edits
  that carry the substantive change back. Falls back to naming the files when no
  snapshot is present. Exit 2 if any source diverged (like ``check``), 0 otherwise.
  """
  groups = cfg.status.get("groups") or {}
  diverged = 0
  for src in select_sources(cfg, args.group or [], args.source or []):
    ...
    if has_doc:
      ...
      print(
        f"  {src.name}: SEAL BROKEN - hand-authored snippet changed (no source doc)"
      )
    for c in cleaned_for(cfg, src.group, src.name):
      ...
      if not snap.is_file():
        print(
          f"    {c['snippet']}: no seal snapshot here - compare it against the "
          "doc by hand"
        )
        continue
      lines = list(
    ...
    if has_doc:
      ...
      print(
        "    -> review the change above; re-seal (seal/include) once it is intended."
      )
  return diverged
