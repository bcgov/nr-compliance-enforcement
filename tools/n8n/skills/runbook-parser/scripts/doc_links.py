#!/usr/bin/env python3
"""Maintain the doc-link registry and the per-group static shims that validators source.

Drives ``config.yaml`` ``status.docs.topics`` plus the generated shims. One tool, no
yq/bash helper -- it reads the config directly:

  1. Validate each topic's URL over HTTP, following redirects, and remove the ones
     whose content is gone (HTTP 404/410). Transient failures (timeouts, 5xx,
     401/403) are KEPT and reported.
  2. Generate each group's shim -- snippets/<group>/url-shim-<group>.sh (the prefix
     separator matches the group's own) -- from assets/doc-url-shim-template.sh, so a
     validator resolves a doc URL with no yq/config at run time.

    python .../scripts/doc_links.py            # validate + prune dead URLs, regen shims
    python .../scripts/doc_links.py --dry-run  # report only; no config/shim writes

Needs internet (the whole point is to confirm the URLs are live). Stdlib only (urllib).
Reuses the parser's config I/O, so the commented spec stays byte-for-byte and only the
status block is rewritten.
"""
...
GONE = {
  404,
  410,
}  # only a definitive "gone" prunes a topic; everything else is kept + reported
DEFAULT_CONFIG = Path(__file__).resolve().parents[3] / "config.yaml"
TEMPLATE = (
  Path(__file__).resolve().parent.parent / "assets" / "doc-url-shim-template.sh"
)
...
def check_url(url: str, timeout: float) -> tuple[bool, str]:
  """Return (gone, detail); gone=True only for a definitive 404/410 (content gone)."""
...
def render_shim(template: str, topics: list) -> str:
  """Fill the @@TOPICS@@ marker with this group's aspect|version -> url table."""
  rows = [
    f'  ["{t.get("aspect", "")}|{t.get("version", "")}"]="{t.get("url", "")}"'
    for t in topics
  ]
  table = "\n".join(rows)
  out: list[str] = []
...
def by_group(topics: list) -> dict[str, list]:
  """Bucket topics by their ``group`` key (missing group becomes the empty string)."""
...
def generate_shims(cfg: Config, topics: list, dry_run: bool) -> None:
  """Write snippets/<group>/url-shim-<group>.sh for each group in the registry."""
...
def main(argv=None) -> int:
  """Validate doc URLs, prune dead ones, and regenerate the per-group shims."""
  ap = argparse.ArgumentParser(
    description="Maintain the doc-link registry + generate the per-group shims."
  )
  ap.add_argument(
    "--config", type=Path, default=DEFAULT_CONFIG, help="config.yaml path"
  )
  ap.add_argument(
  ...
  ap.add_argument(
    "--dry-run",
    action="store_true",
    help="report only; no config rewrite, no shim writes",
  )
  ...
  if removed and not args.dry_run:
    ...
    info(
      f"[dry-run] would remove {len(removed)} dead topic(s); {len(kept)} would remain"
    )
  else:
